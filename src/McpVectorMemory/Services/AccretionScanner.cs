...
public sealed class AccretionScanner
{
    ...
    /// On partial failure, the pending collapse is preserved so the operation can be retried.
    /// </summary>
        ...
        ClusterManager clusters, LifecycleEngine lifecycle)
    {
        ...
        // Create cluster (generates a cluster ID from the collapse ID)
        ...
        {
            // If cluster already exists (from a prior partial attempt), continue with the rest
    ...
    /// Distance metric: 1 - cosineSimilarity(a, b)
    /// minPoints is the minimum number of distinct external neighbors (self excluded).
    /// </summary>
        ...
        List<CognitiveEntry> entries, float epsilon, int minPoints)
    {
        ...
        for (int i = 0; i < entries.Count; i++)
        {
            ...
            while (seedSet.Count > 0)
            {
                ...
                if (labels[q] == 0) // Was noise, now border point
                    labels[q] = clusterId;

                if (labels[q] != -1) continue; // Already assigned
                labels[q] = clusterId;
    ...
    /// <summary>Find all points within epsilon distance of the given point (excluding self).</summary>
        ...
        List<CognitiveEntry> entries, float[] norms, int pointIndex, float epsilon)
    {
        ...
        for (int i = 0; i < entries.Count; i++)
        {
            if (i == pointIndex) continue; // Exclude self
